...
import Initialize_BB11Z_DREAM01 as iBB
...
def likelihood(par):
    ...
    logp, emOut, lFPar, emLF \
        = mymod.likelihood_Emission_LF (par, iBB.tdseries, iBB.lF, 
                        iBB.meteo_data, iBB.meas_data_flow, 
                        iBB.lab_data, iBB.sdData)

    # logp, emOut, cLOut, lFPar \
    #       = mymod.likelihood_SL2 (par,iBB.tdseries, iBB.lF, iBB.meteo_data,
    #                              iBB.meas_data_flow, iBB.lab_data, iBB.sdData)

    return logp
    #return logp, emOut, cLOut, lFPar

parfn = './Results/BB11Z_SL_'
logpsfn = './Results/BB11Z_SL_logps_'

niterations = 5000
converged = False
nchains = 5
d_iter = niterations
d_max = 135000 # only relevant if restart is true... (last file to be read)
restart = False
