...
import numpy as np
from Eval_SL_WM_DREAM02_longterm import *
...
fig1 = plt.figure(1)
plt.plot(iWM.meas_data_flow.index,iWM.meas_data_flow['totF'].values,'o')
...
fig2 = plt.figure(2)
plt.clf()
plt.plot(iWM.tmeas,iWM.meas_data_flow.diff()/iWM.measFreq, 'g.')
for ii in range(len(par_set)):
...
fig3 = plt.figure(3)
plt.plot(iWM.lab_data.index,iWM.lab_data['Cl'],'g.')
for ii in range(len(par_set)):
    plt.plot(iWM.trange[iWM.tmeaslab_ind], cDr_tot[ii][iWM.tmeaslab_ind])
plt.xlabel('year')
plt.ylabel(r'$[{Cl}^-]$ [mg/L]')
plt.title(loc_name)

n2006 = np.where(iWM.trange=='2006-01-01')[0][0]
fig4 = plt.figure(4)
plt.clf()
for ii in range(len(par_set)):
    totF = (cumqDr_tot[ii][n2006:] - cumqDr_tot[ii][iWM.tmeas_ind[0]])
    plt.plot(iWM.trange[n2006:],np.diff(totF))
...
fig5 = plt.figure(5)
for ii in range(len(par_set)):
    plt.plot(iWM.trange[n2006:],cDr_tot[ii][n2006:])
plt.title('long term, outflow concentration')
plt.xlabel('year')
plt.ylabel(r'$[{Cl}^-]$ [mg/L]')

fig6 = plt.figure(6)
[plt.plot(iWM.trange[n2006:],mW_tot[ii][n2006:,-1]/vW_tot[ii][n2006:,-1]) for ii in range(len(par_set))]
plt.title('long term, bulk concentration')
plt.xlabel('year')
plt.ylabel(r'$[{Cl}^-]$ [mg/L]')

fig7 = plt.figure(7)
[plt.plot(iWM.trange[n2006:],mW_tot[ii][n2006:,-1]) for ii in range(len(par_set))]
...
fig8 = plt.figure(8)
plt.clf()
[plt.plot(iWM.trange[n2006:],np.sum(mW_tot[ii][n2006:,:],axis=1)) for ii in range(len(par_set))]
...
fig9 = plt.figure(9)
plt.clf()
[plt.plot(iWM.trange[n2006:],vW_tot[ii][n2006:,-1]) for ii in range(len(par_set))]
...
pset = pd.DataFrame()
for ii in range(len(par_set)):
    pset[ii] = pd.Series(par_set[ii],index=iWM.par_df.columns) 
...
if plot_dists:
    ...
    par_mean = pd.Series(par_mean,index=iWM.par_df.columns)
    par_std = pd.Series(par_std,index=iWM.par_df.columns)
    ...
    for dim in range(ndims):
        ...
        plt.title(iWM.par_df.columns[dim])
...
if plot_pairs:
    ...
    df_par.columns = iWM.par_df.columns
    ...
    for ii in range(len(hicor[0])):
        ...
        plt.xlabel(iWM.par_df.columns[hicor[0][ii]])
        plt.ylabel(iWM.par_df.columns[hicor[1][ii]])
...
dryWeightpm2 = iWM.lF.wetWeight * fDM / iWM.lF.surfArea

# dry density landfill
dryDensity = dryWeightpm2 / iWM.lF.height
...
fig11 = plt.figure(11)
plt.clf()
[plt.plot(iWM.trange[n2006:],np.sum(mW_tot[ii][n2006:,:],axis=1)/dryWeightpm2) for ii in range(len(par_set))]
plt.title('long term, total mass in wastebody')
plt.xlabel('year')
plt.ylabel(r'$M_{{Cl}^-} [g/kg]$')

fig12, ax12 = plt.subplots(1, 1, figsize=(5, 5))
mflow = iWM.meas_data_flow.diff()[1:].T.values[0]
ndims = len(par_set)
colors = sns.color_palette(n_colors=ndims)
...
mconc = iWM.lab_data['Cl'][1:].T.values
stats_c = np.zeros([len(par_set),2])
for ii in range(len(par_set)):
    sconc = cDr_tot[ii][iWM.tmeaslab_ind][1:]
...
# plot qinf
[ax14[0].plot(iWM.trange[n2006+1:],qInf_tot[ii][n2006:]) for ii in range(len(par_set))]
     
[ax14[1].plot(iWM.trange[n2006:], np.maximum(0,(thCL_tot[ii][n2006:]-emLF_tot[ii].cL.thRes)
...
fig17 = plt.figure(17)
plt.clf()
[plt.plot(iWM.trange[n2006:],np.sum(vW_tot[ii][n2006:,:],axis=1)) for ii in range(len(par_set))]
